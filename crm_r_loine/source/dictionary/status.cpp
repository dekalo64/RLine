...
void Status::fillingModel(QSqlQuery &stored)
{
    m_model->clear();
...
void Status::slotSelectRecords(bool actual)
{
    QList<QVariant> list;
    QSqlQuery       stored;
    ...
    stored = this->execStored(currentDatabase(), "ReadAllStatusType", storageHashTable(list));
    ...
    actualRecordsRefresh = actualRecords;
}
...
void Status::slotPushSelectRecordData(void)
{
    QList<QVariant> list;
    QSqlQuery       stored;
    QString         userName;
...
void Status::slotDataChanged(const QModelIndex &index)
{
    if (index.isValid()){
...
void Status::slotGetSelectionModel(const QModelIndex &index)
{
    if (index.isValid()){
        m_selectedItem = true;
    }
